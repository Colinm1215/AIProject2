...
def hill_climbing(problem):
    ...
    while not problem.goal_test(current.state):
        ...
        # Compare the neighbor with shortest SDL to the current node. If closer, make neighbor the current node and append to the path
        if problem.value(neighbor.state) <= problem.value(current.state):
            current = neighbor
            path.append(current)
        # If the chosen neighbor is not closer than the current node to the goal and we are not at the goal state.
        # Indicate that we have reached a local min.
